...
class MultiheadAttention(nn.Module):
    ...
    def __call__(self, kv, q, mask):
        ...
        logits = nn.Dropout(rate=cfg.dropout_rate, deterministic=not cfg.training)(
            logits
        )

        return logits

    def scaled_dot_product_attention(self, key, query, value, mask, dtype, pos_embed):
        """Matmul
        ...
        if pos_embed == "rope":
            ...
            query = jnp.concatenate(
                [
                    get_rope(query[:, :length_q_2, :, :]),
                    get_rope(query[:, length_q_2:, :, :][:, ::-1, ...])[:, ::-1, ...],
                ],
                axis=1,
            )
            key = jnp.concatenate(
                [
                    get_rope(key[:, :length_q_2, :, :]),
                    get_rope(key[:, length_q_2:, :, :][:, ::-1, ...])[:, ::-1, ...],
                ],
                axis=1,
            )
...
class MLP(nn.Module):
    ...
    def __call__(self, x):
        ...
        x = nn.Dropout(rate=cfg.dropout_rate, deterministic=not cfg.training)(x)
        x = netket.nn.cosh(x)
        x = nn.Dense(
            features=cfg.features,
            use_bias=cfg.use_bias,
...
class Encoder(nn.Module):
    ...
    def __call__(self, input_embedding, encoder_mask):
        ...
        for i in range(cfg.layers):
            ...
            # y = nn.InstanceNorm()(x)
            # y = x
            y = MultiheadAttention(config=cfg)(kv=y, q=y, mask=encoder_mask)
            # Add & Norm
            x = y + x
            # y = nn.LayerNorm()(x)
            y = nn.RMSNorm()(x)
...
class EnergyBasedTransformer(nn.Module):
    ...
    def __call__(self, encoder_input_tokens, energy=None):
        ...
        state_embedding = nn.Embed(
            num_embeddings=cfg.n_state, features=features_2, dtype=cfg.dtype
        ...
        if cfg.embed_concat:
            input_embedding = jnp.concatenate([pos_embedding, state_embedding], axis=-1)
        else:
            input_embedding += pos_embedding

        energy_dense = nn.Dense(
            features=features_2,
            use_bias=cfg.use_bias,
            dtype=cfg.dtype,
            kernel_init=cfg.default_kernel_init,
        ...
        energy_emb = nn.Embed(num_embeddings=1, features=batch_size, dtype=cfg.dtype)
        params_energy_emb = energy_emb.init(
            {"params": self.key}, (jnp.zeros(shape=(1,), dtype=jnp.int32))
        )
        ...
        if energy is None:
            ...
            def computeEnergyBasedModel(param, inp):
                ...
                out = enc(input_embedding=inp, encoder_mask=padding_mask)
                out = out[:, -1, :]
                out = final_dense(out)
            ...
            for i in range(10):
                ...
                grad = grad_fun(
                    params_energy_emb, (jnp.zeros(shape=(1,), dtype=jnp.int32))
                )
                params_energy_emb = jax.tree.map(
                    lambda x, y: x - 0.01 * y, params_energy_emb, grad
                )

            energy = params_energy_emb["params"]["embedding"].reshape(batch_size, 1, 1)
        ...
        x = enc(input_embedding=x, encoder_mask=padding_mask)
        x = x[:, -1, :]
        x = final_dense(x)

        if psi_2_intermediate:
            psi_2_intermediate = jnp.concatenate(psi_2_intermediate, axis=1)
...
class EnergyOptimModel(nn.Module):
    ...
    def __call__(self, parameters, inp):
        energy = nn.Embed(num_embeddings=1, features=1, dtype=cfg.dtype)(
            jnp.zeros(shape=(inp.shape[0],), dtype=jnp.int32)
        )
        energy = energy[:, None]
...
if __name__ == "__main__":
    ...
    energy = jax.random.normal(key=key, shape=(inp.shape[0], 1, 1), dtype=cfg.dtype)
