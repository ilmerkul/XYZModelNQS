...
import numpy as np
from ..exact import get_ellist, analytical_energy, analytical_s1sn_z, \
    analytical_s1sn_xy, analytical_m, analytical_s1z
...
class Results:
    ...
    def analytical(self):
        """Analytical solution"""
        ares = {}
        els = get_ellist(self.n, self.h, self.j)
        ...
        if np.abs(self.h) > np.abs(self.j):
            ...
            ares["e"] = analytical_energy(self.n, self.h, self.j, l0)
            ares["xx"] = ares["yy"] = analytical_s1sn_xy(self.n, l0)
            ares["zz"] = analytical_s1sn_z(self.n, l0)
            ares["1z"] = ares["nz"] = analytical_s1z(self.n, l0)
            ares["m"] = analytical_m(self.n, l0)
